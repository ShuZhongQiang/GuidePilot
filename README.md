...
- � **100% 本地处理** - 所有数据和截图完全存储在本地，绝不上传到任何服务器
...
**SaaS 产品教程** - 快速生成用户操作指南， 为用户生成详细的操作步骤说明
**QA 测试报告** - 记录 Bug 复现步骤，方便开发定位问题
**标准作业程序** - 企业 SOP 文档自动化生成
**敏感业务流程** - 银行、医疗、金融等隐私敏感场景（数据完全本地存储，不上传云端）

## 🆚 为什么选择 GuidePilot？

| 特性          | GuidePilot               | Tango / Scribe |
| ----------- | ------------------------ | -------------- |
| **数据隐私**    | ✅ 100% 本地存储，绝不上传         | ❌ 截图上传到第三方服务器  |
| **安装方式**    | ✅ Chrome 扩展，一键安装         | ✅ Chrome 扩展    |
| **AI 文档生成** | ✅ 支持 OpenAI 兼容 API       | ✅ 内置 AI        |
| **多格式导出**   | ✅ Markdown / HTML / JSON | ⚠️ 有限格式        |
| **离线使用**    | ✅ 完全支持                   | ❌ 需要联网         |
| **价格**      | ✅ 开源免费                   | 💰 付费订阅        |

> 🔒 **隐私安全是 GuidePilot 的核心优势**：与 Tango、Scribe 等不同，您的所有操作数据和截图都完全存储在本地浏览器中，不会上传到任何第三方服务器。这对于处理敏感信息（如企业内部系统、客户数据、财务信息）的团队来说至关重要。
...
本项目使用了第三方库，其许可证信息请参阅 [THIRD\_PARTY\_NOTICES.md](THIRD_PARTY_NOTICES.md)。
